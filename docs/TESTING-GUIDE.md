...
| 1b.3 | `message_handler.cpp:421` | **EXPECTED, accepted.** Resolved 27.07: the engine reads only `main_menu/gui/messagetypes.txt`, so no mod file can add a message type without replacing vanilla's 1348 entries. Our dead file was deleted. Cost is this line plus no popup; the action works | — (unfixable) |
| 1b.5 | `modifier_type.cpp:1294` Missing Icon | **EXPECTED, accepted.** Icons are by filename convention (`gfx/interface/icons/modifier_types/<key>.dds`); vanilla omits some of its own. Fixable only by shipping a .dds | — (cosmetic) |
...
| 3.3b | any | **Sub-vassals count as ours** | Give your vassal a vassal of its own inside a goal region. That ground must satisfy the end requirement, must NOT be seized by the failsafe, and must be shaded as your realm on the map. Before 27.07 all three were wrong — `is_subject_of` only matched a direct vassal | `top_overlord_or_this` across 46 sites |
| 3.3c | any | **A vassal may hold the seats** | Hand Samarkand or Khanbaliq to a vassal. The end requirement must still be satisfied. Before 27.07 it was not, and the 1545 failsafe could not fix it either — it never takes land from your own subjects, so the phase was locked open forever | `mr_in_claimant_realm` |
| 3.3d | any | **End requirements read as a checklist** | P1 shows 2 ticks, P2 4, P3 9 — each one short sentence with its own green/red mark. If a phase instead shows its tooltip text *plus* a raw clause dump underneath, a requirement has been wrapped in one mega-tooltip again | one `custom_tooltip` per requirement |
